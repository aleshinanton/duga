...
- **EPIC-33: TUI Redesign — Modern, Dense, Professional Terminal UI.**  Complete rearchitecture of the duga TUI from a single-pane transcript+input layout into a modern multi-pane terminal application. 197 tests, 0 regressions. Key additions:
  - **Theme system** (`theme.rs`): `Theme` struct with named `ThemeColors` (bg, surface, primary, success, warning, error, muted, text, text_dim, border, accent). Dark and light presets. All widgets reference `theme.colors.*` instead of raw `Color::Rgb(…)`. Loaded from `tui.theme.name` config.
  - **Layout engine** (`layout.rs`): `LayoutManager` computing `PaneRects` (header, chat, sidebar, banner, input, footer) from terminal dimensions. Responsive breakpoint at 120 cols — sidebar collapses below threshold. Configurable sidebar width (15-40%).
  - **Header widget** (`header.rs`): Single-row bar showing app name (cyan bg), model, status dot (green ● / yellow ◌ / red ✗), token count (≈ chars÷3.5), estimated cost, and session name. Truncates gracefully on narrow terminals.
  - **Chat panel** (`chat.rs`): Transcript items rendered as bordered message cards with role-colored titles (You=cyan, Assistant=green, System=blue, Error=red). Markdown rendering restored for assistant messages (headings, bold, italic, code blocks, lists, blockquotes). Thinking blocks nest inside assistant cards. Blank rows + thin separators between cards. Line buffer pre-allocation and item-based virtualization via `Transcript::estimate_visible_range()`.
  - **Footer widget** (`footer.rs`): Context-aware shortcut bar. Shows different shortcuts for Idle state (F1 Help, Ctrl+L Clear, Tab Focus, Ctrl+S Sessions, q Quit), Running state (Ctrl+C Cancel, Ctrl+G Steer), and Cancelling state (Ctrl+R Retry).
  - **Sidebar container** (`sidebar.rs`): Split pane (50/50) with reasoning panel (top) and event log (bottom). Each panel independently collapsible via `r`/`e` shortcuts. Empty state message when both collapsed. Zero-op when sidebar width=0 (responsive collapse).
  - **Reasoning panel** (`reasoning_panel.rs`): Shows thinking content with word count and duration tracking (from first `LlmThinkingDelta` timestamp). Auto-starts on first delta, auto-finishes when text tokens arrive or run finishes. Resets on transcript clear.
  - **Event log** (`event_log.rs`): `EventLog` with `VecDeque<LogEntry>` ring buffer (max 200 entries, FIFO eviction). Captures tool calls (started ●, success ✓, failure ✗), errors, memory compressions (⚠), and loop delegations (→). Rendered in sidebar with severity icons and level-colored text.
  - **Focus model** (`focus.rs`): `Focus::Chat | Sidebar | Input | Overlay` enum. Tab/Shift+Tab cycles Chat → Sidebar → Input (Overlay is modal, not in cycle). Dedicated shortcuts: `i` jumps to Input, `r` toggles reasoning + focuses Sidebar, `e` toggles events + focuses Sidebar, `Esc` returns to Chat or closes overlay. Overlay focus restores previous focus on close. Keys route to correct widget based on focus.
  - **Error banners** (`banner.rs`): Full-width dismissible notifications above the input area. Color-coded by level (Error=red, Warn/Cancel=yellow, Info=blue). Auto-dismiss after configurable timeout (default 5s). Dismiss with Enter key. New banner replaces old (no stacking).
  - **Input enhancements**: Character counter `[N/500]` in the input block title. Color changes at thresholds: normal (dimmed), 80-95% fill (yellow), >95% (red). Stronger visual focus indicator when Input is focused. Context-aware placeholder ("Steering…" during run).
  - **Kill switch**: `tui.use_new_layout: bool` (default `true`). Set to `false` to fall back to original single-pane layout.
  - **Config changes** (all `#[serde(default)]`, backward compatible): `tui.theme.name`, `tui.show_sidebar`, `tui.sidebar_width_pct`, `tui.show_footer`, `tui.show_header`, `tui.responsive_breakpoint`, `tui.input_max_chars`, `tui.banner_auto_dismiss_secs`, `tui.event_log_max_entries`, `tui.use_new_layout`.
